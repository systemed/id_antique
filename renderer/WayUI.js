...
declare("iD.renderer.WayUI", [iD.renderer.EntityUI], {
	...
	redraw:function() {
		...
		// Add hitzone sprite
		if (drawn) {
			var hit=this.recordSprite(this.targetGroup('hit').createPolyline(coords).setStroke( { width:maxwidth, color:[0,0,0,0] } ));
			hit.source=this;
...
//		console.log("WayUI.entityMouseEvent:",event.type);	//,event.gfxTarget.source
